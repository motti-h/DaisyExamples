...
std::vector<std::vector<float>> scales_one_octave = { huseyni_voltages_one_octave, huzam_voltages_one_octave };
std::vector<std::string> scales_names = { "Huseyni", "Huzam" };
Parameter cvIn1;
float valOut=0;
float closestNote=0;
                   ...
                   size_t                    size)
{
    ...
    hw.seed.dac.WriteValue(DacHandle::Channel::ONE,static_cast<uint16_t>((DAC_MAX/5.f) * closestNote));
}
...
int main(void)
{
    ...
    std::string str2 = "closest note: " + std::to_string(static_cast<uint32_t>(DAC_MAX*closestNote));
    char *      cstr2 = &str2[0];
    hw.display.SetCursor(0, 9);
    hw.display.WriteString(cstr2,Font_6x8,true);

    
    notesVector = scales_one_octave.at(encoder % 2);

    std::string str3 = "maqam: " + scales_names.at(encoder % 2);
    char *      cstr3 = &str3[0];
